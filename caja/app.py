def get_fecha_hoy():
    """Devuelve la fecha actual en formato dd-mm-YYYY usando la zona horaria de Monterrey."""
...
INV_URL_BASE = "https://inventario.celularescreditofacil.xyz/"
...
def crear_pendiente(
    ...
    for u in involucrados or []:
        cur.execute(
            "INSERT INTO pendientes_involucrados (pendiente_id, usuario, estado) VALUES (?, ?, 'pendiente')",
            (pid, u),
        )
    conn.commit()
    ...
    try:
        ...
        cur.execute(
            "UPDATE pendientes_involucrados SET ultimo_correo=?, recordatorios=COALESCE(recordatorios,0)+1 WHERE pendiente_id=?",
            (ahora_mx(), pid),
        )
        conn.commit()
    except Exception:
...
def obtener_sucursales():
    with read_conn() as conn:
        ...
        try:
            cur.execute(
                "SELECT DISTINCT sucursal FROM usuarios WHERE sucursal IS NOT NULL AND sucursal<>'' ORDER BY sucursal"
            )
            rows = cur.fetchall()
            if rows:
    ...
    try:
        ...
        try:
            with smtplib.SMTP_SSL(
                SMTP_HOST, 465, context=ssl.create_default_context(), timeout=25
            ) as s:
                s.login(SMTP_USER, SMTP_PASS)
                s.send_message(msg)
            return True, ""
...
def recalc_desglose_dia(conn, fecha_dia, sucursal):
    cur = conn.cursor()
    cur.execute(
        "DELETE FROM ventas_desglose WHERE fecha LIKE ? AND sucursal = ?",
...
def crear_pendiente(
    conn, tipo, referencia="", sucursal="", detalle_dict=None, creado_por=""
):
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO pendientes (tipo, referencia, sucursal, fecha, detalle, estado, creado_por) VALUES (?, ?, ?, ?, ?, 'pendiente', ?)",
...
def get_admin_emails(conn):
    cur = conn.cursor()
    cur.execute(
        "SELECT correo FROM usuarios WHERE tipo='admin' AND correo IS NOT NULL AND correo<>''"
    )
...
def ensure_ventas_desglose_schema(conn):
    ...
    if "venta_id" not in cols:
        ...
        conn.commit()
    cur.execute(
        "CREATE UNIQUE INDEX IF NOT EXISTS uq_vd_venta_id ON ventas_desglose(venta_id)"
    )
    cur.execute(
        "CREATE INDEX IF NOT EXISTS idx_vd_fecha_sucursal ON ventas_desglose(fecha, sucursal)"
    )
    conn.commit()
...
def get_emails_by_usernames(conn, nombres):
    ...
    cur = conn.cursor()
    cur.execute(
        f"SELECT correo FROM usuarios WHERE nombre IN ({qmarks}) AND correo IS NOT NULL AND correo<>''",
        tuple(nombres),
    )
    return [r[0] for r in cur.fetchall()]
...
def get_watchers_emails(conn):
    cur = conn.cursor()
    cur.execute(
        "SELECT correo FROM usuarios WHERE recibe_todos_pendientes=1 AND correo IS NOT NULL AND correo<>''"
    )
    return [r[0] for r in cur.fetchall()]
...
def is_watcher(conn, user):
    cur = conn.cursor()
    cur.execute(
        "SELECT 1 FROM usuarios WHERE nombre=? AND recibe_todos_pendientes=1", (user,)
    )
    ...
    if usuario and tipo in ["consulta", "reparto"]:
        ...
        if not inicio_sesion_str:
            ...
            if ahora - inicio_sesion > timedelta(hours=4):
                session.clear()
                flash(
                    "🔒 Tu sesión ha expirado después del turno de 4 horas. Por favor inicia sesión de nuevo."
                )
                return redirect("/")
...
def login():
    if request.method == "POST":
        ...
        with read_conn() as conn:
            cur = conn.cursor()
            cur.execute(
                "SELECT id, contraseña, tipo FROM usuarios WHERE nombre = ?", (nombre,)
            )
        ...
        if usuario and usuario[1] == contraseña and usuario[2] == tipo_usuario:
            ...
            return render_template(
                "login.html", error="Credenciales incorrectas", tipo=tipo_usuario
            )
...
def panel_admin():
    ...
    return render_template(
        ...
        bloqueos_pendientes=bloqueos_pendientes,
    )
...
def crear_pendiente_view():
    ...
    if request.method == "POST":
        ...
        try:
            detalle = (
                json.loads(detalle_txt)
                if detalle_txt.strip().startswith("{")
                else {"texto": detalle_txt}
            )
        except Exception:
...
def ver_pendientes():
    ...
    return render_template(
        ...
        soy_admin=(session.get("tipo") == "admin"),
    )
...
def pendiente_realizado(pid):
    ...
    with db_conn() as conn:
        ...
        if quedan == 0:
            ...
            cur.execute(
                "SELECT tipo, referencia, sucursal, fecha FROM pendientes WHERE id=?",
                (pid,),
            )
            row = cur.fetchone()
            if row:
                ...
                if tipo == "feria":
                    ...
                    if bid is not None:
                        cur.execute(
                            "UPDATE bloqueos_feria SET autorizado=1 WHERE id=?", (bid,)
                        )
                    else:
                        if suc:
                            if fecha_dia:
                            ...
                            if r2:
                                cur.execute(
                                    "UPDATE bloqueos_feria SET autorizado=1 WHERE id=?",
                                    (r2[0],),
                                )
...
def pendientes_resumen_json():
    ...
    with read_conn() as conn:
        ...
        cur.execute(
            "SELECT tipo, COUNT(*) FROM pendientes WHERE estado='pendiente' GROUP BY tipo"
        )
    ...
    with read_conn() as conn:
        ...
        cur.execute(
            """SELECT COUNT(*)
                       ...
                       WHERE pi.usuario=? AND pi.estado='pendiente' AND p.estado='pendiente'""",
            (u,),
        )
        total = int(cur.fetchone()[0] or 0)
...
def registro():
    if request.method == "POST":
        ...
        if contrasena != confirmar:
            return render_template(
                "registro.html", error="Las contraseñas no coinciden."
            )

        patron = r"^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*()_\-+=?<>.,]).{6,}$"
        if not re.match(patron, contrasena):
            return render_template(
                "registro.html", error="La contraseña no cumple con los requisitos."
            )
        ...
        with db_conn() as conn:
            ...
            while cur.fetchone():
                ...
                usuario_generado = f"{tipo_u}_{nombre_base}_{codigo}"
                cur.execute(
                    "SELECT 1 FROM usuarios WHERE nombre = ?", (usuario_generado,)
                )
            ...
            crear_pendiente(
                conn,
                "registro_usuario",
                referencia=usuario_generado,
                sucursal="",
                detalle_dict=detalle,
                creado_por=usuario_generado,
            )

        return render_template(
            "registro.html",
            success="Tu solicitud fue enviada. Un administrador debe autorizarla.",
        )
...
def recuperar():
    if request.method == "POST":
        ...
        if not contacto:
            return render_template(
                "recuperar.html",
                error="Debes ingresar tu correo, teléfono o nombre de usuario.",
            )
        ...
        with db_conn() as conn:
            ...
            if not row:
                return render_template(
                    "recuperar.html",
                    error="No se encontró ninguna cuenta con ese dato.",
                )
            usuario, correo, telefono = row
            if not correo:
                return render_template(
                    "recuperar.html", error="Tu cuenta no tiene un correo registrado."
                )
            cur.execute("""
            ...
            """)
            cur.execute(
                "DELETE FROM codigos_verificacion WHERE usuario = ?", (usuario,)
            )
            cur.execute(
                """
        ...
        if ok:
            ...
            return render_template(
                "recuperar.html", error=f"Error enviando el correo: {err}"
            )
...
def verificar_codigo():
    if request.method == "POST":
        ...
        if not usuario or not codigo_ingresado or not nueva_contrasena:
            return render_template(
                "verificar_codigo.html", usuario=usuario, error="Faltan datos."
            )
        with read_conn() as conn:
            cur = conn.cursor()
            cur.execute(
                "SELECT codigo, expira FROM codigos_verificacion WHERE usuario = ? LIMIT 1",
                (usuario,),
            )
            row = cur.fetchone()
        if not row:
            return render_template(
                ...
                usuario=usuario,
                error="No hay un código activo. Vuelve a solicitarlo.",
            )
        codigo_guardado, expira = row
        if int(time.time()) > int(expira):
            return render_template(
                "verificar_codigo.html", usuario=usuario, error="El código ha expirado."
            )
        if codigo_ingresado != str(codigo_guardado):
            return render_template(
                "verificar_codigo.html", usuario=usuario, error="Código incorrecto."
            )
        with db_conn() as conn:
            cur = conn.cursor()
            cur.execute(
...
def enviar_codigo():
    ...
    if not dato:
        return render_template(
            "enviar_codigo.html", error="Ingresa un correo, teléfono o nombre."
        )
    codigo = "".join(random.choices(string.digits, k=6))
    expira = int(time.time()) + 15 * 60
    with db_conn() as conn:
        ...
        if not correo:
            return render_template(
                "enviar_codigo.html", error="Tu cuenta no tiene un correo registrado."
            )
        cur.execute("""
            CREATE TABLE IF NOT EXISTS codigos_verificacion(
                usuario TEXT,
    ...
    if ok:
        ...
        return render_template(
            "enviar_codigo.html", error=f"No se pudo enviar el correo: {err}"
        )
        ...
        with db_conn() as conn:
            ...
            if tipo == "feria":
                ...
                if bid is not None:
                    ...
                    fec = (
                        detalle.get("fecha")
                        or (fecha_pend[:10] if len(fecha_pend) >= 10 else "")
                        or ""
                    ).strip()
                    if suc:
                        if fec:
...
def rechazar_pendiente(pid):
    ...
    with db_conn() as conn:
        ...
        if row[0] != "pendiente":
            return jsonify(success=False)
        cur.execute(
            "UPDATE pendientes SET estado='rechazado', autorizado_por=?, fecha_autorizacion=? WHERE id=?",
            (admin, ahora_mx(), pid),
        )
        conn.commit()
    return jsonify(success=True)
...
def logout():
    registrar_log(
        session.get("usuario", "Desconocido"),
        session.get("tipo", "Desconocido"),
        "Cerró sesión",
    )
    session.clear()
    return redirect("/")
...
def log_actividad():
    ...
    with read_conn() as conn:
        ...
        if origen:
            cur.execute(
                "SELECT usuario, tipo, accion, fecha, origen FROM log_actividad WHERE origen = ? ORDER BY id DESC",
                (origen,),
            )
        else:
            cur.execute(
                "SELECT usuario, tipo, accion, fecha, origen FROM log_actividad ORDER BY id DESC"
            )
        rows = cur.fetchall()
...
def gastos():
    ...
    try:
        fecha_filtro_db = datetime.strptime(fecha_qs_iso, "%Y-%m-%d").strftime(
            "%d-%m-%Y"
        )
    except Exception:
    ...
    if request.method == "POST":
        ...
        if tipo_usuario == "admin":
            ...
            try:
                fecha_post_db = datetime.strptime(fecha_post_iso, "%Y-%m-%d").strftime(
                    "%d-%m-%Y"
                )
            except Exception:
...
def ventas():
    ...
    if fecha_input:
        try:
            fecha_filtro = datetime.strptime(fecha_input, "%Y-%m-%d").strftime(
                "%d-%m-%Y"
            )
        except ValueError:
    ...
    with read_conn() as conn:
        ...
        if tipo_usuario == "admin":
            cur.execute(
                "SELECT * FROM ventas WHERE fecha LIKE ? AND sucursal = ?",
...
def cobrar():
    ...
    if tipo_usuario == "admin":
        sucursal = (
            request.form.get("sucursal_manual") or session.get("sucursal", "")
        ).strip()
        fecha_manual = (request.form.get("fecha_manual") or "").strip()
        if fecha_manual:
            try:
                fecha_dt = datetime.strptime(fecha_manual, "%Y-%m-%d").replace(
                    tzinfo=tz
                )
                fecha_dt = fecha_dt.replace(
                    hour=ahora.hour, minute=ahora.minute, second=ahora.second
                )
            except ValueError:
                fecha_dt = ahora
        else:
    ...
    with db_conn() as conn:
        cur = conn.cursor()
        cur.execute(
            "SELECT COUNT(*) FROM ventas WHERE fecha LIKE ? AND sucursal = ?",
            (f"{fecha_dia}%", sucursal),
        )
        ...
        if ventas_hoy == 0:
            ...
            cur.execute(
                "SELECT autorizado FROM bloqueos_feria WHERE fecha = ? AND sucursal = ? ORDER BY id DESC LIMIT 1",
                (fecha_dia, sucursal),
            )
            ...
            if not es_feria and not autorizado_hoy:
                with write_tx() as tx:
                    tx.execute(
                        "INSERT OR IGNORE INTO bloqueos_feria (sucursal, fecha, motivo, autorizado) VALUES (?, ?, ?, 0)",
                        (sucursal, fecha_dia, "Primera venta no fue FERIA"),
                    )
                    bid = tx.execute(
                        """SELECT id FROM bloqueos_feria
                           WHERE sucursal=? AND fecha=? AND motivo=? AND autorizado=0
                           ORDER BY id DESC LIMIT 1""",
                        (sucursal, fecha_dia, "Primera venta no fue FERIA"),
                    ).fetchone()[0]
                    ...
                    crear_pendiente(
                        ...
                        sucursal,
                        {
                            "motivo": "Primera venta no fue FERIA",
                            "sucursal": sucursal,
                            "fecha": fecha_dia,
                            "origen": "caja:/cobrar",
                            "usuario": usuario,
                    ...
                    with write_tx() as tx:
                        tx.execute(
                            ...
                            (
                                sucursal,
                                fecha_dia,
                                "Feria no coincide con la salida de ayer",
                            ),
                        )
                        bid = tx.execute(
                            """SELECT id FROM bloqueos_feria
                        ...
                        crear_pendiente(
                            ...
                            sucursal,
                            {
                                "motivo": "Feria no coincide con la salida de ayer",
                                "sucursal": sucursal,
                                "fecha": fecha_dia,
                                "origen": "caja:/cobrar",
                                "usuario": usuario,
    ...
    usd_usado_mxn = min(usd_mxn_entregado, restante)
    restante = round(restante - usd_usado_mxn, 2)
    efectivo_usado = min(efectivo, restante)
    restante = round(restante - efectivo_usado, 2)
    tarjeta_usada = min(tarjeta, restante)
    restante = round(restante - tarjeta_usada, 2)
    ...
    with write_tx() as conn:
        ...
        for idx, item in enumerate(carrito):
            ...
            if imei_val:
                try:
                    cur.execute(
                        "SELECT name FROM sqlite_master WHERE type='table' AND lower(name)='articulos'"
                    )
                    if cur.fetchone():
                        cur.execute("PRAGMA table_info(articulos)")
                        cols = {r[1].lower() for r in cur.fetchall()}
                        # Detectar cómo se llama la columna del IMEI
                        for cand in ("imei", "imei1", "imei_1", "serial", "sn"):
                            if cand in cols:
                                imei_col = cand
                                break
                        else:
                            imei_col = None
                        if "estado" in cols and imei_col:
                            cur.execute(
                                f"UPDATE articulos SET estado='Vendido' WHERE TRIM({imei_col})=TRIM(?)",
                                (imei_val,),
                            )
                        else:
                            print(
                                "WARN Caja:/cobrar -> 'articulos' sin columnas requeridas (estado/IMEI). No se actualiza inventario."
                            )
                    else:
                        print(
                            "WARN Caja:/cobrar -> Tabla 'articulos' no existe en DB de Caja. No se actualiza inventario."
                        )
                except Exception as e:
                    print(
                        f"ERROR Caja:/cobrar actualizando articulos IMEI {imei_val}: {type(e).__name__}: {e}"
                    )
...
def bloqueos_pendientes_json():
    ...
    with read_conn() as conn:
        cur = conn.cursor()
        cur.execute(
            "SELECT COUNT(*) FROM pendientes WHERE tipo='feria' AND estado='pendiente' AND fecha LIKE ?",
            (fecha_hoy + "%",),
        )
        total = int(cur.fetchone()[0] or 0)
        cur.execute(
            "SELECT DISTINCT sucursal FROM pendientes WHERE tipo='feria' AND estado='pendiente' AND fecha LIKE ? ORDER BY sucursal",
            (fecha_hoy + "%",),
        )
...
def ticket():
    ...
    fecha = datetime.now(ZoneInfo("America/Monterrey")).strftime(
        "%d/%m/%Y %H:%M %S (Hora local)"
    )
    ...
    if request.method == "POST":
        ...
        with read_conn() as con:
            ...
            for d in DENOM:
                delta = diferencias[d]
                if delta > 0:
                    actual_caja = cur.execute(
                        "SELECT cantidad FROM caja WHERE denom = ?", (d,)
                    ).fetchone()
...
def confirmar_cambio():
    ...
    total_objetivo = raw_data.get(
        "total_objetivo", sum(data["objetivos"][d] * d for d in DENOM)
    )
    total_actual = raw_data.get(
        "total_actual", sum(data["actuales"][d] * d for d in DENOM)
    )
    total_repuesto = raw_data.get(
        "total_repuesto", sum(data["repuesto"][d] * d for d in DENOM)
    )
    ...
    if request.method == "POST" and not ya_registrado:
        with write_tx() as con:
            ...
            for d in DENOM:
                ...
                if delta > 0:
                    cur.execute(
                        "UPDATE caja SET cantidad = cantidad - ? WHERE denom = ?",
                        (data["repuesto"][d], d),
                    )
                elif delta < 0:
                    cur.execute(
    ...
    with write_tx() as conn:
        ...
        if request.method == "POST":
            for denom, cantidad in request.form.items():
                cur.execute(
                    "UPDATE caja SET cantidad = ? WHERE denom = ?", (cantidad, denom)
                )
            conn.commit()
            registrar_log(
                session["usuario"], session["tipo"], "Modificó valores de 'Caja Actual'"
            )
...
def guardar_ciclo():
    ...
    with write_tx() as conn:
        ...
        for d in datos:
            ...
            cur.execute(
                "UPDATE fondo SET cantidad_actual = ?, cantidad_repuesta = ?, monto_dado = monto_dado + ?, monto_recibido = monto_recibido + ? WHERE denom = ?",
                (
                    total,
                    d["repuesto"],
...
def ver_ciclos():
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        cur = conn.cursor()
        cur.execute(
            "SELECT id, sucursal, fecha, datos FROM ciclos ORDER BY date(fecha) DESC, id DESC"
        )
        rows = cur.fetchall()
    ...
    for row in rows:
        ...
        objetivos = {
            int(k) if isinstance(k, str) else k: int(v or 0)
        ...
        ciclos.append(
            {
                ...
                "total_recibido": int(
                    datos.get("recibir", datos.get("total_recibir", 0)) or 0
                ),
...
def ver_fondo():
    ...
    with read_conn() as conn:
        ...
        cur = conn.cursor()
        cur.execute(
            "SELECT denom, SUM(cantidad) as total FROM caja GROUP BY denom ORDER BY denom ASC"
        )
        datos = cur.fetchall()
...
def modificar_ciclo(ciclo_id):
    ...
    with sqlite3.connect(DB_PATH) as conn:
        cur = conn.cursor()
        cur.execute(
            "SELECT sucursal, fecha, datos FROM ciclos WHERE id = ?", (ciclo_id,)
        )
        row = cur.fetchone()
        ...
        if request.method == "POST":
            ...
            cur.execute(
                "UPDATE ciclos SET datos = ? WHERE id = ?",
                (str(nuevos_datos), ciclo_id),
            )
            conn.commit()
            return redirect("/ver-ciclos")

    return render_template(
        "modificar_ciclo.html",
        ciclo_id=ciclo_id,
        sucursal=sucursal,
        fecha=fecha,
        datos=datos,
...
def faltantes_json():
    ...
    return jsonify({"faltan": faltan, "todas_registradas": len(faltan) == 0})
        ...
        with write_tx() as conn:
            ...
            for denom in DENOM:
                ...
                if actual < objetivo:
                    ...
                    cur.execute(
                        "UPDATE fondo SET cantidad_actual = cantidad_actual - ? WHERE denominacion = ?",
                        (sugerido, denom),
                    )
...
def guardar_feria():
    ...
    with write_tx() as conn:
        ...
        for val in objetivos:
            ...
            cur.execute(
                "UPDATE caja SET cantidad = ? WHERE denom = ?", (nuevo_valor, val)
            )
        ...
        cur.execute(
            "INSERT INTO ciclos (sucursal, fecha, datos) VALUES (?, ?, ?)",
            (sucursal, fecha, json.dumps(datos)),
        )
...
def eliminar_ciclo(ciclo_id=None):
    ...
    return redirect(
        f"/ver-ciclos?sucursal={suc}&fecha={fec}" if (suc or fec) else "/ver-ciclos"
    )
...
def feria_admin():
    ...
    sucursal_sel = request.values.get("sucursal") or (
        session.get("sucursal")
        if session.get("sucursal") in sucursales
        else sucursales[0]
    )
...
def nota():
    ...
    with write_tx() as conn:
        ...
        if es_admin:
            ...
            if (
                session.get("sucursal")
                and session["sucursal"] not in sucursales_disponibles
            ):
        ...
        if request.method == "POST":
            ...
            if texto and fecha_post and sucursal_post:
                ...
                registrar_log(
                    session["usuario"],
                    tipo,
                    f"Registró una nota en {sucursal_post} ({fecha_post})",
                )
...
def eliminar_nota():
    ...
    with write_tx() as conn:
        ...
        if tipo == "consulta":
            cur.execute(
                "DELETE FROM notas WHERE id = ? AND sucursal = ?",
                (nota_id, session.get("sucursal", "")),
            )
        else:
...
def api_reporte_excel_resumen():
    ...
    try:
        with read_conn() as conn:
            ...
            if has_col(cur, "ventas", "tipo_pago"):
                ...
                for col, var in (
                    ...
                    if has_col(cur, "ventas", col):
                        cur.execute(
                            f"SELECT IFNULL(SUM({col}),0) FROM ventas WHERE fecha LIKE ? AND sucursal=? AND (eliminado IS NULL OR eliminado=0)",
                            (f"{fecha_dia}%", sucursal),
                        )
            ...
            cur.execute(
                "SELECT IFNULL(SUM(monto),0), COUNT(*) FROM gastos WHERE fecha LIKE ? AND sucursal=?",
                (f"{fecha_dia}%", sucursal),
            )
            g_total, g_count = cur.fetchone() or (0, 0)

            cur.execute(
                "SELECT IFNULL(SUM(monto),0), COUNT(*) FROM anticipos WHERE fecha LIKE ? AND sucursal=?",
                (f"{fecha_dia}%", sucursal),
            )
            ...
            if has_col(cur, "dolares", "monto"):
                cur.execute(
                    "SELECT IFNULL(SUM(monto),0), IFNULL(SUM(cantidad),0) FROM dolares WHERE fecha LIKE ? AND sucursal=?",
                    (f"{fecha_dia}%", sucursal),
                )
            ...
            if has_col(cur, "fondo_caja", "monto"):
                cur.execute(
                    "SELECT monto FROM fondo_caja WHERE fecha LIKE ? AND sucursal=? ORDER BY id DESC LIMIT 1",
                    (f"{fecha_dia}%", sucursal),
                )
                row_f = cur.fetchone()
            ...
            if has_col(cur, "notas", "texto"):
                cur.execute(
                    "SELECT texto FROM notas WHERE fecha LIKE ? AND sucursal=?",
                    (f"{fecha_dia}%", sucursal),
                )
                for (texto,) in cur.fetchall():
                    for x in re.findall(r"\$\s*([\d,]+(?:\.\d{1,2})?)", texto or ""):
                        try:
                            total_notas += float(x.replace(",", ""))
                        except Exception:
                            pass
        ...
        return jsonify(
            ...
            ventas_count=int(v_count or 0),
            efectivo=round(efectivo, 2),
            tarjeta=round(tarjeta, 2),
            dolar_en_mxn=round(dolar_mxn, 2),
            mixto=round(mixto, 2),
            gastos_total=round(g_total, 2),
            gastos_count=int(g_count or 0),
            anticipos_total=round(a_total, 2),
            anticipos_count=int(a_count or 0),
            dolares_mxn=round(d_mxn_total, 2),
            dolares_pzas=int(d_pzas or 0),
            fondo_caja=round(fondo, 2),
            notas_total=round(total_notas, 2),
...
def reporte_excel():
    ...
    with read_conn() as conn:
        ...
        cur.execute(
            "SELECT motivo, monto FROM gastos WHERE fecha LIKE ? AND sucursal = ?",
            (f"{fecha}%", sucursal),
        )
        gastos = cur.fetchall()
        ...
        cur.execute(
            "SELECT tipo, cantidad, monto FROM dolares WHERE fecha LIKE ? AND sucursal = ?",
            (f"{fecha}%", sucursal),
        )
        dolares = cur.fetchall()

        cur.execute(
            "SELECT monto FROM anticipos WHERE fecha LIKE ? AND sucursal = ?",
            (f"{fecha}%", sucursal),
        )
    ...
    output.seek(0)
    return send_file(
        output, download_name=f"corte_{sucursal}_{fecha}.xlsx", as_attachment=True
    )
