        ...
        if ventas_hoy == 0:
            ...
            if not es_feria and not autorizado_hoy:
                with write_tx() as tx:
                    ...
                    crear_pendiente(
                        tx, "feria", ref, sucursal,
                        {"motivo":"Primera venta no fue FERIA","sucursal":sucursal,"fecha":fecha_dia,"origen":"caja:/cobrar","usuario":usuario},
                        usuario, involucrados=None
                    )
            ...
            if es_feria and not autorizado_hoy:
                ...
                if a_centavos(monto_venta_feria) != a_centavos(total_feria_ayer):
                    with write_tx() as tx:
                        ...
                        crear_pendiente(
                            tx, "feria", ref, sucursal,
                            {"motivo":"Feria no coincide con la salida de ayer","sucursal":sucursal,"fecha":fecha_dia,"origen":"caja:/cobrar","usuario":usuario},
                            usuario, involucrados=None
                        )
                    return render_template("venta_bloqueada.html",
    ...
    with write_tx() as conn:
        ...
        for idx, item in enumerate(carrito):
            ...
            cur.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?, ?, ?)
            """, (usuario, sucursal,
                  item.get("descripcion", ""), item.get("tipo", ""), item.get("concepto", ""),
                  ref_item if ref_item else referencia_general, (item.get("imei") or "").strip(),
                  safe_float(item.get("precio", 0)), item.get("tipo_pago", ""), fecha,
                  e_i, t_i, usd_i, round(tipo_cambio, 4)))

            imei_val = (item.get("imei") or "").strip()
            if imei_val:
                try:
                    cur.execute("SELECT name FROM sqlite_master WHERE type='table' AND lower(name)='articulos'")
                    if cur.fetchone():
                        ...
                        if "estado" in cols and imei_col:
                            ...
                            print("WARN Caja:/cobrar -> 'articulos' sin columnas requeridas (estado/IMEI). No se actualiza inventario.")
                    else:
                        print("WARN Caja:/cobrar -> Tabla 'articulos' no existe en DB de Caja. No se actualiza inventario.")
                except Exception as e:
                    print(f"ERROR Caja:/cobrar actualizando articulos IMEI {imei_val}: {type(e).__name__}: {e}")
